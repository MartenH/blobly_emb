...
fn test_the_fb_loop_subtracts_the_io_counter() {
	// the real generated node: this is about the emitted dispatch, not a fixture
	gen := os.join_path(@VMODROOT, 'examples', 'system_full', 'nodes', 'domain', 'gen', 'loom_gen.v')
	src := os.read_file(gen) or {
		// the file is generated by `make -C examples/system_full/nodes/domain gen`; absent in a
		// clean tree, and skipping is right — a missing generated file is not a regression here
		return
	}
	lines := strip_comments(src)
	mut dispatch := ''
	...
	assert dispatch != '', '${gen}: no sched.run_profiled* dispatch emitted'
	assert dispatch == 'sched.run_profiled_excl(trace_clock, io_exec_clock)', '${gen}: the FB dispatch is `${dispatch}` — with io points and trace level="all" it must be run_profiled_excl(trace_clock, io_exec_clock), or handler load charges io preemption again'
	assert clock_fn, '${gen}: io_exec_clock() does not read C.io_exec_us() — the dispatch would subtract something else'
}
