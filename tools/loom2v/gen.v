...
fn emit_io_target_entry(m Model, ioc_idx map[string]int, with_load bool, load_slot int) []string {
	...
	g << '\t\t// all io frozen while comm/app ran). Re-anchor so next_us is never more than one'
	g << '\t\t// period ahead: the sleep is bounded to <= one period, worst-case recovery is one.'
