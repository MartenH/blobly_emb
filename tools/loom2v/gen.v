...
fn trace_dbc(cmd_id u32, rsp_id u32, stat_id u32, detail_id u32) string {
	...
	l << ''
	if detail_id != 0 {
		l << 'BO_ ${detail_id} LoadDetail: 8 SUT'
	...
	l << 'BO_ ${cmd_id} TraceCmd: 8 Tester'
	...
	l << ''
	l << 'BO_ ${stat_id} HandlerStat: 8 SUT'
	...
	l << ''
	if detail_id != 0 {
		l << 'CM_ BO_ ${detail_id} "One core\'s processor load over 100ms/1s/10s windows + per-period overrun count.";'
	}
	l << 'CM_ BO_ ${cmd_id} "Trace capture control, host -> target. Config cmd/rsp, NOT UDS.";'
	l << 'CM_ BO_ ${rsp_id} "Trace capture ack + status, target -> host.";'
	l << 'CM_ BO_ ${stat_id} "Live per-handler stats push: response time (= CPU on a polled core), us.";'
	l << 'CM_ SG_ ${cmd_id} opcode "1 arm | 2 start | 3 stop | 4 reset | 5 set_push | 6 dump | 7 status";'
	l << 'CM_ SG_ ${cmd_id} core_mask "bit i selects core i; 0 = the receiving/default core (single-core target)";'
	l << 'CM_ SG_ ${cmd_id} handler_filter "0xFFFF = all handlers, else a single handler_id";'
	l << ''
	l << 'VAL_ ${cmd_id} opcode 1 "arm" 2 "start" 3 "stop" 4 "reset" 5 "set_push" 6 "dump" 7 "status" ;'
	l << 'VAL_ ${rsp_id} opcode_echo 1 "arm" 2 "start" 3 "stop" 4 "reset" 5 "set_push" 6 "dump" 7 "status" ;'
	l << 'VAL_ ${rsp_id} result 0 "ok" 1 "bad_opcode" 2 "unsupported" 3 "not_ready" 4 "busy" ;'
	l << 'VAL_ ${rsp_id} state 0 "idle" 1 "capturing" 2 "full" 3 "frozen" ;'
	return l.join('\n') + '\n'
}
