...
fn main() {
	...
	// Multi-core (P3a): two shared scratch slots reserved after telem's. (1) SYSTEM-WIDE FREEZE
	// flag: when any core's capture overruns it sets this; every core's capture hook sees it and
	// triggers its own ring, so a dump is a coherent cross-core snapshot. (2) ARM GENERATION: the
	// trace owner bumps it on arm/start/reset; each partition observes the change and resets its
	// OWN capture origin (start/base), so a re-armed capture is timestamped from the new origin —
	// the origin lives in per-partition state the owner must not touch (single-writer isolation).
	trace_trig_slot := slot_core.len
	trace_arm_slot := slot_core.len + 1
	...
	if trace_multicore {
		...
		if trace_budget_us > 0 {
			...
			// a coherent cross-core snapshot. trigger() is idempotent (its pending/state guards),
			// so calling it every capture while the flag is set acts exactly once per core.
			glue << '\tif dt_us > ${trace_budget_us} {'
			glue << '\t\tosal.scratch_set(${trace_trig_slot}, 1) // ask every core to freeze'
			glue << '\t}'
			glue << '\tif osal.scratch_get(${trace_trig_slot}) != 0 {'
			glue << '\t\tt.buf.trigger()'
			glue << '\t}'
		}
		glue << '}'
		...
		glue << '\tif e_start > t.busy_end { // idle gap since the last busy span'
		glue << '\t\tmut gap := e_start - t.busy_end'
		glue << '\t\tif gap > 0xFFFF {'
		glue << '\t\t\tgap = 0xFFFF'
		glue << '\t\t}'
		glue << '\t\tt.buf.push(trace.new_idle(trace.reason_yield, u32(t.busy_end - t.base), u16(gap)))'
		glue << '\t}'
	...
	for part, clist in by_part {
		...
		if !target_on && !trace_inline && !trace_multicore {
			...
			glue << '\tmut last_arm := osal.scratch_get(${trace_arm_slot})'
			glue << '\tfor {'
			// The owner bumps the arm generation on arm/start/reset; reseat THIS core's capture
			// origin (single-writer — the owner never touches per-partition state) so a re-armed
			// capture's records are timestamped from the new origin, not the old process epoch.
			glue << '\t\tif osal.scratch_get(${trace_arm_slot}) != last_arm {'
			glue << '\t\t\tlast_arm = osal.scratch_get(${trace_arm_slot})'
	...
	// --- partition_trace (P3a): the single trace-bus owner. Runs the TraceCmd/Rsp handshake for
	//     every core's ring (handle_cmd enforces the core mask), and on `dump` streams each frozen
	//     ring as one self-describing ISO-TP block (pack_block: header + records), sequentially —
	//     isotp.Link handles one transfer at a time, so busy() gates the next block. The rings are
	//     owned by run() and passed as a base pointer; CpuLoad is handled by partition_telem. ---
	if trace_multicore {
		glue << ''
		...
		glue << '\t\t\t\t\tosal.scratch_set(${trace_trig_slot}, 0) // new session: clear the system freeze'
		glue << '\t\t\t\t\t// bump the arm generation so each partition reseats its own capture origin'
		glue << '\t\t\t\t\tosal.scratch_set(${trace_arm_slot}, osal.scratch_get(${trace_arm_slot}) + 1)'
		glue << '\t\t\t\t}'
		...
		glue << '\t\t\t\t\trspb, do_dump, addressed := trace.handle_cmd(mut tb, c, u8(cc))'
		glue << '\t\t\t\t\tif !addressed {'
		glue << '\t\t\t\t\t\tcontinue'
		glue << '\t\t\t\t\t}'
		glue << '\t\t\t\t\tif do_dump && !dump_busy {'
		...
		glue << '\t\t\t\t\t}'
		glue << '\t\t\t\t\tif do_dump && dump_busy { // a dump is already in flight'
		glue << '\t\t\t\t\t\trf.data[1] = trace.result_busy'
		glue << '\t\t\t\t\t}'
		glue << '\t\t\t\t\tch.send(rf)'
		glue << '\t\t\t\t}'
