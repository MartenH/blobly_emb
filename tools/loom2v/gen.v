...
//   * from == to  -> LOCAL: a cell field in the partition state (direct memory)
//   * from != to  -> IOC channel gen.<snake(name)>_ch (transport per signal)
//
// Convention: a signal name P maps to the value type `sig.P`. Each handler's glue
// snapshots its inputs into an `In`, calls the FB method, and sinks its `Out`.
//
