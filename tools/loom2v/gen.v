...
// eth_e2e_filtered: has any NON-eth frame E2E config — eth frames' E2E is the
// appended trailer (docs/someip.md), derived + ecumodel-gated, and generates
// no comm.e2e call this rung, so eth keys must drive neither the DBC-backed
// position validation nor the comm.e2e import.
fn eth_e2e_filtered(m Model) (map[string]bool, bool) {
	mut eth_fks := map[string]bool{}
	for fr in m.eth_frames {
		eth_fks[snake(fr.name)] = true
	}
	mut has_e2e := false
...
fn main() {
	...
	// Per-PDU COM behaviour ([[frame]]), rebound to the existing locals. Eth
	// frame keys are excluded from the legacy DBC-backed E2E machinery
	// (eth_e2e_filtered): their trailer is derived + ecumodel-gated.
	eth_fks, has_e2e := eth_e2e_filtered(m)
	has_secoc := m.frames.secoc_on.len > 0
	...
	if has_e2e || has_secoc {
		...
		for fk, _ in m.frames.e2e_on {
			if fk in eth_fks {
				continue // eth trailer: positions already derived + gated by ecumodel
			}
