...
fn emit_satellite_images(m Model, doc toml.Doc, producers []Producer, ecu string) {
	...
	for part, rel in m.part.image {
		...
		// TraceHostCtx{}: a satellite IMAGE is its own build (docs/multi-image.md), never the
		// host bridge-owner shape.
		...
		// cross-core bulk pools with an endpoint on THIS satellite: same shared-window wrappers
		// the owner emits (identical offsets), so both sides address the same pool.
		g << emit_bulk_glue(m.bulk, m.part, part)
