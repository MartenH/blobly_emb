/* boards/h723/eth.c — NUCLEO-H723ZG Ethernet MAC/DMA + LAN8742A RMII driver.
 *
 * The register-level hardware half of the on-target TCP/IP stack (docs/net.md).
 * Same DWC_ether_qos MAC as the H735-DK (RM0468, H72x/H73x): the MAC/DMA/MDIO and
 * descriptor code below is IDENTICAL to boards/h735dk/eth.c. Only two things are
 * board-specific and differ here:
 *   1. RMII pin map — NUCLEO-144 (MB1364) puts TX_EN/TXD0 on PG11/PG13 (the H735-DK
 *      used PB11/PB12); everything else matches. See eth_pins_init.
 *   2. MDIO clock divider — this board's HCLK is 200 MHz (SYSCLK 400 / HPRE 2, see
 *      boards/h723/board.c), so the 150-250 MHz range (DIV102) keeps MDC under the
 *      LAN8742's 2.5 MHz limit: 200/102 = 1.96 MHz. (The H735-DK ran 275 MHz HCLK
 *      and used DIV124.)
...
/* --- descriptor + buffer memory (static; no heap). 32-byte aligned. Placed by the
 * linker in D2 AHB SRAM (.eth_dma at 0x30000000 — the ETH DMA is an AHB master and
 * cannot reach the DTCM the rest of RAM sits in). */
typedef struct {
 ...
 * H735 P1 bring-up bugs were found; keep them. */
 ...
 * for the MDC: HCLK is 200 MHz here (boards/h723/board.c: SYSCLK 400 / HPRE 2), so
 * the 150-250 MHz range (DIV102) keeps MDC under the PHY's 2.5 MHz limit —
 * 200/102 = 1.96 MHz. */
...
/* ---------------------------------------------------------- pins + clocks --- */
/* mux one pin to AF11, very-high speed. */
...
static void eth_pins_init(void) {
	/* NUCLEO-H723ZG (MB1364) RMII: RX/MDIO/REF_CLK on A/C (as the H735-DK), but
	 * TX_EN/TXD0 on PORT G (PG11/PG13) — the NUCLEO-144 default, not PB11/PB12. */
...
int eth_init(const uint8_t mac[6]) {
	 ...
	 * RX ring and crowds out our own reply traffic. */
	if (phy_bringup() != 0) {
		return -2;
	}
	...
	/* 6. speed/duplex: NO blocking autoneg wait. One non-blocking sync applies the
	 * real mode if autoneg already settled (or the 100M/full fallback if not), and
	 * the app's periodic eth_link_up() poll converges MACCR to the actual mode. */
	phy_sync_maccr();
	/* ACS+CST: strip pad/FCS in hardware so RDES3's length is the frame WITHOUT the
	 * 4-byte CRC. */
...
int eth_send(const uint8_t *frame, uint32_t len) {
	...
	/* N-1 rule: also require the NEXT slot (the tail target) to be free, or a tail
	 * that wrapped onto a still-OWNed descriptor reads as "no work" and wedges TX. */
	...
	/* NO hardware checksum insertion (CIC=0): NetX computes checksums in software;
	 * letting the MAC also insert would overwrite the correct value. */
	d->des3 = DESC_OWN | TDES3_FD | TDES3_LD;
	tx_idx = (tx_idx + 1u) % ETH_TX_DESC_CNT;
	/* DSB: the descriptor stores (normal memory) must be visible BEFORE the
	 * tail-pointer doorbell (device memory), or the DMA fetches OWN=0 and idles. */
	__DSB();
...
uint32_t eth_recv(uint8_t *buf, uint32_t buf_size) {
	 ...
	 * would be indistinguishable from "ring empty". 0 means ONLY "nothing pending". */
	for (;;) {
		eth_desc_t *d = &rx_desc[rx_idx];
		if ((d->des3 & DESC_OWN) != 0u) {
			return 0; /* DMA still owns it — nothing received */
		}
		__DMB(); /* order the frame/length reads after the OWN observation */
		uint32_t len = 0;
		...
		/* recycle the descriptor back to the DMA (DSB: OWN visible before doorbell). */
