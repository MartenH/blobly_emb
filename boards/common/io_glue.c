 ...
 *
 * boards/common/io_glue.c — GENERIC target glue, config-independent (the comm_glue.c
 * pattern), the CAN-comm + io + multi-thread-load superset. Promoted from the per-example
 * copies so a CAN+io node (system_full zone_a) reuses it instead of copying. a small indexed
