...
tools/    BUILD-TIME only (heap OK): dbc2cfg, benches, candb, threadx_amp PoC
config/   ecu.toml, *.dbc
```
...
- **No runtime heap.** In `app/`, `comm/`, `loom/`: no `string`, no `map`, no
  growable `[]T`, no closures. Only fixed arrays (`[N]T`), value structs, static
...
- **Generated code.** `comm/com/dbc_gen.v` is generated by `tools/dbc2cfg` — never
  hand-edit it; changes belong in the generator.
