...
#include "xcore_gen.h" /* the GENERATED slot contract (../h755_threadx/gen — one generator run
                      * owns the cross-core map; this satellite image consumes it) */

/* --- boot handshake -------------------------------------------------------------------
 * Park until the CM7 signals clocks-ready (xcore.h): the kernel's SysTick is configured
 ...
 * The pool lives at a fixed SRAM4 address (xcore.h), NOT in either image's bss. The channel
 ...
 * The generated wrappers publish via xcore_pub with slots from gen/xcore_gen.h. */
#define XCORE_POOL ((xioc_t *)XCORE_IOC_ADDR)

void xcore_ioc_init(void) {
	for (int i = 0; i < XCORE_IOC_N; i++) {
		xioc_init(&XCORE_POOL[i]);
	}
}

void xcore_pub(int i, uint32_t a, uint32_t b) {
	if (i >= 0 && i < XCORE_IOC_N) {
		xioc_write(&XCORE_POOL[i], a, b);
	}
}

/* --- wide channels (xioc_n) ------------------------------------------------------------
 * Signals past the {a,b} pair (3+ fields, sub-u32 types, or `valid`) ride size-
 * proportional xioc_n channels in the XCORE_XW window; per-signal offsets are generated
 * (gen/xcore_gen.h XCORE_XW_<SIG>_OFF) and the generated boot inits each channel this image
 * writes. Same plain-store discipline as the pair pool. */
static uint32_t g_boot_seed; /* set by xcore_layout_retract, boot's first act */

void xcore_xw_init(uint32_t off, uint32_t words) {
	/* seq space seeded per BOOT from the retained epoch (set in xcore_layout_retract,
	...
	xioc_n_init((xioc_n_t *)(XCORE_XW_ADDR + off), words, g_boot_seed);
}

void xcore_pub_n(uint32_t off, const uint32_t *src) {
	xioc_n_write((xioc_n_t *)(XCORE_XW_ADDR + off), src);
}

/* xcore_layout_retract — the satellite's FIRST act at boot, before any channel init:
...
void xcore_layout_retract(void) {
	...
	*(volatile uint32_t *)XCORE_LAYOUT_ACK_ADDR = 0u;
	...
	uint32_t e = *(volatile uint32_t *)XCORE_EPOCH_ADDR + 1u;
	*(volatile uint32_t *)XCORE_EPOCH_ADDR = e;
	g_boot_seed = (e * 2654435761u) | 1u;
	__asm__ volatile("dmb" ::: "memory");
}

/* xcore_layout_publish — the satellite's half of the layout handshake: after every
...
void xcore_layout_publish(void) {
	...
	uint32_t req = *(volatile uint32_t *)XCORE_LAYOUT_REQ_ADDR;
	uint32_t ack = req ^ XCORE_LAYOUT_ID;
	if (ack == 0u) ack = 0x4C594F31u; /* 'LYO1' — never the retraction sentinel (r16) */
	*(volatile uint32_t *)XCORE_LAYOUT_ACK_ADDR = ack;
	__asm__ volatile("dmb" ::: "memory");
}

/* --- dtrace: the two-core trace handoff (xcore.h) -----------------------------------------
...
void xcore_trace_service(void) {
	...
	uint32_t op = c[1];
	if (op == XCORE_TRC_OP_ARM) {
		trace_arm();
		c[3] = 0u;
	} else if (op == XCORE_TRC_OP_SNAP) {
		trace_freeze();
		c[3] = trace_snapshot((unsigned char (*)[8])XCORE_TRC_BUF_ADDR, XCORE_TRC_MAX_REC);
	}
	...
	c[XCORE_TRC_SVC_IDX] = (uint32_t)trace_now_us();
	__asm__ volatile("dmb" ::: "memory");
 ...
 * The bulk pool lives in the H755 shared window; the generated code externs xcore_bulk_base()
 ...
 * first 4 bytes, XCORE_STRESS_K*seq+i in the rest — tear/corruption detectable by the consumer),
...
/* Cross-core CpuLoad (xcore.h XCORE_LOAD_ADDR): publish THIS core's per-mille processor load into its
...
void xcore_load_pub(int core, uint16_t pm) {
	if (core >= 0 && core < 8) {
		((volatile uint16_t *)XCORE_LOAD_ADDR)[core] = pm;
	}
}
...
void xcore_bulk_produce(void) {
	/* SINGLE-POOL DEMO: "xfer" is the only cross-core pool, so it sits at offset 0 =
	 * XCORE_BULK_ADDR and the geometry (4 x 256) matches ecu.toml [[bulk]] and the generated
	...
	/* BENCH: while the CM7 raises the burst flag (bulkperf), produce flat-out so it can measure the
	 * pool's raw cross-core throughput. WORD-wise fill (a realistic bulk COPY, not the byte-wise
	 * seq*K integrity pattern whose per-byte multiply would dominate) and NO doorbell — the CM7
	 * spin-drains during the bench, so ringing IRQ125 per block would only interrupt the measuring
	 * core. Spins on a full pool; exits when the CM7 clears the flag. */
	...
	for (uint32_t i = 4; i < 256u; i++) {
		b[i] = (uint8_t)(seq * XCORE_STRESS_K + i);
	}
	...
	uint32_t rl = HSEM->RLR[XCORE_BULK_DOORBELL_SEM];          /* fast-take: lock for this core */
	HSEM->R[XCORE_BULK_DOORBELL_SEM] = rl & HSEM_R_COREID_Msk; /* release -> HSEM1 interrupt on the CM7 */
}
