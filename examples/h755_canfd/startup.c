/* Minimal Cortex-M7 startup for the STM32H755 (M7 core) — vector table + reset.
 * No ST startup file, no CMSIS init: set up memory and jump to the V main loop. */
#include <stdint.h>

extern uint32_t _sidata, _sdata, _edata, _sbss, _ebss, _estack;
/* Call V's `fn main` body directly (main__main), NOT V's int main(int,char**)
 * wrapper — that wrapper runs _vinit (hosted-OS arg/global setup) and faults
 * bare-metal. main.v only calls C shims, so it needs none of it. */
