...
pub const src_frame2_id = u32(0x101)
pub const src_frame2_dlc = u8(8)

// Rpm: 0|16 @1 (Intel) unsigned (1.0,0.0) "rpm"
pub fn src_frame2_rpm_raw(data [64]u8) u64 {
	mut raw := u64(0)
	for i in 0 .. 16 {
		g := 0 + i
		byte_idx := g / 8
...
pub fn src_frame2_rpm_phys(data [64]u8) f64 {
	raw := src_frame2_rpm_raw(data)
	return f64(raw) * 1.0 + 0.0
}
pub fn src_frame2_rpm_set_raw(mut data [64]u8, raw u64) {
	for i in 0 .. 16 {
		g := 0 + i
		byte_idx := g / 8
		if byte_idx >= 64 {
...
pub const dst_frame2_id = u32(0x201)
pub const dst_frame2_dlc = u8(8)

// Rpm: 48|16 @1 (Intel) unsigned (1.0,0.0) "rpm"
pub fn dst_frame2_rpm_raw(data [64]u8) u64 {
	mut raw := u64(0)
	for i in 0 .. 16 {
		g := 48 + i
		byte_idx := g / 8
...
pub fn dst_frame2_rpm_phys(data [64]u8) f64 {
	raw := dst_frame2_rpm_raw(data)
	return f64(raw) * 1.0 + 0.0
}
pub fn dst_frame2_rpm_set_raw(mut data [64]u8, raw u64) {
	for i in 0 .. 16 {
		g := 48 + i
		byte_idx := g / 8
		if byte_idx >= 64 {
